...
public interface DomainValueRepository extends CrudRepository<DomainValue, Long>, QuerydslPredicateExecutor<DomainValue> {
    @Query(value = "select count(d) from DomainValue d where d.domain.id=?1 AND d.state=?2")
    Long getCountDomainsValuesByIdAndState(Long pIdDominio, String pState);

    @Query(value = "select d from DomainValue d where d.domain.id=?1 AND d.state=?2")
    List<DomainValue> getDomainsValuesPageableByIdAndState(Long pIdDominio, String pState, Pageable pPageable);
    ...
    @Query(value = "select d from DomainValue d where d.domain.codeDomain = ?1 and d.state = ?2")
    List<DomainValue> getDomainValueByDomainCodeAndState(String pDomainCode, String pState);

    @Query(value = "select d from DomainValue d where d.domain.codeDomain = ?1 and d.charValueExtra=?2 and d.state = ?3")
    ...
    @Query(value = "select d from DomainValue d where d.domain.codeDomain=?1 and d.codeValue=?2 and d.state=?3")
    Optional<DomainValue> getDomainValueByDomainCodeAndCodeValue(String pDomainCode, String pCodeValue, String pState);

    @Query(value = "select d from DomainValue d where d.domain.codeDomain=?1 and d.charValue=?2 and d.state=?3")
    ...
    @Query(value = "select d from DomainValue d where d.domain.typeDomain=?1 and d.state = ?2")
    List<DomainValue> getDomainValueByDomainAndState(String pTypeDomain, String pState);

    @Query(value = "select count(d) from DomainValue d where d.domain.codeDomain=?1 and d.state=?2")
    ...
    @Query(value = "select d from DomainValue d where d.domain.codeDomain=?1 and d.codeValue in ?2 and d.state=?3 order by d.charValue")
